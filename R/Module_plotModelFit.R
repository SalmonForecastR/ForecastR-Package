...
#' @param options
#' @param fc.add
#' @param axis.label  label for the value axis (in the app this is extracted from the data file, with user option to modify)
#'
#' @return
#' @export
#'
...
plotModelFit <- function(fit.obj, options= list(plot.which = "all",age.which="all",plot.add=FALSE),axis.label = "Abundance",fc.add=NULL,formula.show = TRUE,tracing = FALSE){
...
bg.col <- rgb(1, 0, 0,bg.fade)
	...
	if(formula.show){title(main= fit.obj[[age.plot]]$formula,line=0.75,cex.main=1.2) }
...
if("fitted.values" %in% names(fit.obj[[age.plot]]) ){
	...
	plot(fit.obj[[age.plot]]$run.yrs,fit.obj[[age.plot]]$obs.values , bty="n", type="o",cex=0.8,pch=19,col="darkblue",
					xlab="Run Years", ylab= axis.label , ylim=plot.lims,xlim=xlim.use)
